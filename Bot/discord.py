...
async def on_message_delete(message: nextcord.Message):
    ...
    async with session.get(
            f"https://guildcord.deutscher775.de/{message.guild.id}?token={config.MASTER_TOKEN}") as channel_request:
    ...
    if message.channel.id in channel_json["config"]["channels"]["discord"]:
        ...
        async with session.get(
                f"https://guildcord.deutscher775.de/{message.guild.id}?token={config.MASTER_TOKEN}") as log_request:
...
async def on_message_edit(before: nextcord.Message, after: nextcord.Message):
    session = aiohttp.ClientSession()
    async with session.get(
            f"https://guildcord.deutscher775.de/{before.guild.id}?token={config.MASTER_TOKEN}") as channel_request:
        channel_json = await channel_request.json()
        channel_request.close()
    if before.channel.id in channel_json["config"]["channels"]["discord"]:
        ...
        async with session.get(
                f"https://guildcord.deutscher775.de/{before.guild.id}?token={config.MASTER_TOKEN}") as log_request:
            log_json = await log_request.json()
            await client.get_channel(log_json["config"]["logs"]["discord"]).send(embed=embed)
            await session.close()
...
async def on_message(message: nextcord.Message):
    ...
    try:
        async with session.get(
                f"https://guildcord.deutscher775.de/{message.guild.id}?token={config.MASTER_TOKEN}") as discord_channel_request:
        ...
        if message.channel.id in discord_channel["config"]["channels"]["discord"]:
            global blacklist
            async with session.get(
                    f"https://guildcord.deutscher775.de/{message.guild.id}?token={config.MASTER_TOKEN}") as blacklist_reqeust:
            ...
            for word in blacklist["config"]["blacklist"]:
                ...
                global allowed_ids
                async with session.get(
                        f"https://guildcord.deutscher775.de/{message.guild.id}?token={config.MASTER_TOKEN}") as allowed_ids_request:
                ...
                if not message.author.bot or message.author.id in allowed_ids["config"]["allowed-ids"]:
                    if not message.attachments:
                        await session.post(
                            f"https://guildcord.deutscher775.de/update/{message.channel.guild.id}?message_content={message.content}&"
...
async def register(interaction: nextcord.Interaction):
    ...
    try:
        ...
        async with session.post(f"https://guildcord.deutscher775.de/create?endpoint={interaction.guild.id}") as create_request:
        ...
        async with session.post(
                f"https://guildcord.deutscher775.de/update/{interaction.channel.guild.id}?channel_discord={interaction.channel.id}&"
        ...
        async with session.post(
                f"https://guildcord.deutscher775.de/token/{interaction.guild.id}?master_token={config.MASTER_TOKEN}") as token_request:
        ...
        await interaction.edit_original_message(content=
            f"Created enpoint: https://guildcord.deutscher775.de/{interaction.guild.id}\n"
...
async def help(interaction: nextcord.Interaction):
    embed = nextcord.Embed(title="Guildcord", description="API Docs: https://guildcord.deutscher775.de")
...
async def delete(interaction: nextcord.Interaction):
    ...
    await session.delete(
        f"https://guildcord.deutscher775.de/delete/{interaction.guild.id}?token={config.MASTER_TOKEN}")
    await session.close()
...
async def setlog(interaction: nextcord.Interaction, channel: nextcord.TextChannel):
    ...
    session = aiohttp.ClientSession()
    await session.post(
        f"https://guildcord.deutscher775.de/update/{interaction.guild.id}?token={config.MASTER_TOKEN}"
...
async def gen(interaction: nextcord.Interaction):
    ...
    session = aiohttp.ClientSession()
    async with session.post(
            f"https://guildcord.deutscher775.de/token/{interaction.guild.id}?master_token={config.MASTER_TOKEN}") as token_gen:
