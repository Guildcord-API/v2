...
class Client(commands.CommandsClient):
            ...
            for server in os.listdir(f"{pathlib.Path(__file__).parent.resolve()}/revolt_servers"):
                try:
                    ...
                    endpoint_request = requests.get(
                        f"https://guildcord.deutscher775.de/{endpoint}?token={config.MASTER_TOKEN}")
                            ...
                            for attachment in message_attachments:
                                await channel.send(content=attachment,
                                                   masquerade=revolt.Masquerade(name=author_name,
                                                                                        ...
                                                                                        colour=color))
                            post = requests.post(
                                f"https://guildcord.deutscher775.de/read/{endpoint}?token={config.MASTER_TOKEN}&read_revolt=true")
                            post.close()
                        else:
                            await channel.send(content=content,
    ...
    async def on_message(self, message: revolt.Message):
        if not message.author.bot:
            ...
            if message.content.lower() == f"{prefix}help":
                embed = revolt.SendableEmbed(title="Guildcord", description="API Docs: https://guildcord.deutscher775.de/docs\n\n"
        ...
        try:
            ...
            if message.channel.id in requests.get(
                    f"https://guildcord.deutscher775.de/{endpoint}?token={config.MASTER_TOKEN}").json()[
                "config"]["channels"][
                "revolt"]:
                try:
    ...
    async def register(self, ctx, endpoint: int = None):
        ...
        try:
            ...
            await ctx.reply(f"Enpoint exists already: https://guildcord.deutscher775.de/{endpoint}")
            return
        # webhook = await message.channel.create_webhook(name="Guildcord")
        requests.post(
            f"https://guildcord.deutscher775.de/update/{endpoint}?channel_revolt={ctx.message.channel.id}&"
            f"&token={config.MASTER_TOKEN}")
        await ctx.reply(f"Updated enpoint: https://guildcord.deutscher775.de/{endpoint}")
    ...
    async def allow(self, ctx, allowid: str = None):
        ...
        if endpoint:
            requests.post(
                f"https://guildcord.deutscher775.de/update/{endpoint}?allowed_ids={allowid}&token={config.MASTER_TOKEN}")
            await ctx.reply(f"Updated enpoint: https://guildcord.deutscher775.de/{endpoint}")
        else:
    ...
    async def set_logs(self, ctx, channel: str = None):
        ...
        if endpoint:
            requests.post(
                f"https://guildcord.deutscher775.de/update/{endpoint}?"
    ...
    async def set_logs(self, ctx):
        ...
        if endpoint:
            requests.post(
                f"https://guildcord.deutscher775.de/update/{endpoint}?channel_revolt={ctx.channel.id}&token={config.MASTER_TOKEN}")
            await ctx.reply(f"Updated enpoint: https://guildcord.deutscher775.de/{endpoint}")
        else:
            await ctx.reply(":x: - Your endpoint could not be found.")
